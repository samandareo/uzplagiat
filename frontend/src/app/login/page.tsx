  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response?.status === 422) {
        setError(t('auth.error_login_invalid'));
      } else {
        setError(err.response?.data?.detail || t('auth.error_login_failed'));
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      >
        ...
        <div className="text-center mb-10 mt-6">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 mb-3">{t('auth.login_title')}</h2>
